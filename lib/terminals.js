...
// The working directory a session was last operating in. Claude records its cwd
// on every entry, and after a `cd` (into a subdir or git worktree) the tail
// differs from the head — which is also why the session's files get re-homed to a
// different project folder. We read only the file's tail (sessions can be many
// MB) and return the last recorded cwd, so a drifted session can be resumed from
// the directory it actually lives in.
function sessionRecordedCwd(folder, id) {
  const file = path.join(projectsDir(), folder, id + '.jsonl');
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        // A partial first line from the 64KB window — skip it.
      }
    }
  } catch (_) {}
  return null;
  ...
  if (isClaude) {
    ...
    if (hasContinue) {
      ...
      if (sessionExists(folder, chat.sessionId)) {
        ...
        if (other) {
          // The session physically lives under a different project folder —
          // usually because Claude `cd`'d into a subdir/worktree mid-session,
          // which re-homes its files. `--resume` only finds it when launched from
          // a cwd that encodes to that folder, so recover by relaunching in the
          // directory the session was last operating in.
          args.push('--resume', chat.sessionId);
          const realCwd = sessionRecordedCwd(other, chat.sessionId);
          if (realCwd && isDir(realCwd) && encodeProject(realCwd) === other) {
            cwd = realCwd;
            resume = 'resume';
            notice =
              `\r\n\x1b[90m[phantum] resuming from ${realCwd} ` +
              `(this session had moved there)\x1b[0m\r\n`;
          } else {
            warning =
